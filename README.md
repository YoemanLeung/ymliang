...
The Chandra Cycle 27 label in Research funding links to the [user-selected ADS proposal record](https://ui.adsabs.harvard.edu/abs/2025cxo..prop.6948L/abstract), shared by the homepage and HTML/PDF CV.

The homepage opens with the full-square Alps selfie and identity/contact panel to the right of the title (stacked below the title on mobile). The separate About section holds the biography. The CV/appointments/education section comes before Publications; navigation and section numbers follow that order. The 3D page uses the Alps selfie explicitly supplied by the user; the CV uses the user-selected blue-background suit portrait (`yongming-liang-suit.jpg`). Public copies have personal metadata removed, with identical decoded pixels and preserved color profiles where present. Original files remain unchanged. No private slide decks, job-application CV PDFs, private science catalogs, reviewer reports, or original proposal documents are included. Only aggregated telescope totals and the brief funding attribution are published. Talks link to public event information where verified; slide downloads can be added after selecting the exact public versions.

The cosmic background uses real positions from the public Mini-Millennium galaxy catalogue, with adaptive, locally oriented smoothing and no visible legend. A single 379,216-byte binary supplies the geometry; the desktop slice renders 10,942 tracers and mobile renders a deterministic 6,039-tracer subset. It uses one draw call, a 30 fps cap, and pixel-ratio caps of 1.5 (desktop) / 1.25 (mobile). The initial device width selects the particle budget. Motion gently changes the viewing angle of a fixed snapshot; it pauses when hidden or offscreen and starts paused for reduced-motion users. Failure to load the asset or initialize WebGL leaves all academic content accessible. This is an illustrative galaxy-tracer view, with neither a claimed gas density nor a COSMOS-Web data association.
